...
| 体检信号（来自三方报告） | 通常适合 | 建议命令 / 参数 | 需用户确认 |
|---|---|---|---|
...
| kn_scan `待沉淀项目 > 0` | spec-distill | 按其 SKILL.md 6 步 `sync` 逐项目处理 | 是（选系统 + 拆知识点 + 阶段检查点） |
| kn_scan `待沉淀项目 = 0` | —（跳过沉淀阶段） | 报告记"无需处理" | 否 |
...
2. **知识沉淀**（spec-distill）——把 SpecIn 新项目落到 `10-Work/知识库/`。
3. **策展收尾**（wiki-curate）——补「用途」、挂孤儿、收件箱归类、最后 lint。
...
- **项目落哪个系统**（沉淀）：交给 spec-distill 的 AskUserQuestion 关卡，入口不代替决定。
