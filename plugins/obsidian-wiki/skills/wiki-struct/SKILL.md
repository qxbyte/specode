...
本 skill 是**结构层**。同套件还有：`spec-distill`（SpecIn → 10-Work/知识库 知识沉淀 + MEMORY）、`wiki-curate`（方法论伞 + 内容向 ingest/curate/lint + 写作规范/红线/模板 doctrine）。三者各管一摊：结构 → wiki-struct，知识沉淀 → spec-distill，内容策展与方法论 → wiki-curate。
