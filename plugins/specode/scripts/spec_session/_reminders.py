...
此 spec 的可写文档：
  ...
  • design.md（含「测试策略」/「正确性属性 → 验证：需求 x.y」章节）
  • implementation-log.md（如有）

请评估用户本次输入是否涉及以下变更：

- 需求 / 验收标准调整 → 先 Edit `requirements.md` 或 `bugfix.md`
- 架构 / 接口 / 数据模型决策 / 任务拆分 / 测试要点 → 先 Edit `design.md`
...
本 turn 即将结束。如果你在本 turn 内修改了源代码，请自检以下两项：

1. `implementation-log.md` 是否记录？ —— 实现说明、设计偏离、技术决策
2. `design.md` 接口契约是否变化？ —— 若改了，同步 Edit
