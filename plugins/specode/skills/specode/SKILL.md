...
每个 phase-gate 节点必须**调用 Claude Code 内置 `AskUserQuestion` 工具**呈现选择器；工具自动渲染上下键导航 + 回车提交 + ESC 取消 + "Other" 自定义输入。**严禁**自己在 chat 输出 markdown 列表 + "请回复编号"；**严禁**自己加 `Type something` / `Chat about this` 等保留位（工具内置 Other）；**严禁**等待用户回复文本编号。
...
→ 8 个场景的完整 `AskUserQuestion` 调用模板详见 `references/prompts.md`；常量库实现在 `spec_session.py` 的 `SELECTOR_PROMPTS` 字典。
...
→ 完整调用模板详见 `references/prompts.md`。
