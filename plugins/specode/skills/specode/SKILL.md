...
文件优先的规范驱动工作流。`requirements.md` / `bugfix.md` / `design.md` / `tasks.md`（末尾自带 `## 测试要点` 章节）/ `implementation-log.md` 是事实源；代码改动总是滞后于文档落地。所有 hook 都是**提示式注入**，永远不阻断；hook 注入失败或缺失时，本 SKILL.md 的硬约束仍然完整有效。
...
→ 8 个场景的完整 `AskUserQuestion` 调用模板详见 `references/selectors.md`；常量库实现在 `spec_session.py` 的 `SELECTOR_PROMPTS` 字典。
...
| `doc-confirm-{requirements,bugfix,design,tasks}` | A | 对应文档生成后 | 需求/设计/任务/缺陷确认 |
| `tasks-execution` | A | tasks.md 确认后 | 执行方式 |
...
| 文档 | 何时更新 |
|---|---|
| `requirements.md` / `bugfix.md` | 需求 / 验收标准 / 缺陷范围调整。改动后**同 turn**更新 `tasks.md` 末尾 `## 测试要点` 章节。 |
| `design.md` | 架构 / 接口 / 数据模型决策 |
| `tasks.md` | 任务范围 / 状态推进 `[ ]` → `[~]` → `[x]`；末尾 `## 测试要点` 章节每条 SHALL 一行，供测试人员快速了解验证场景 |
...
- `references/templates.md` — 5 份文档模板、EARS SHALL 写法、traceability 规范、tasks.md 测试要点章节
