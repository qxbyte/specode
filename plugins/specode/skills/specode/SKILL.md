...
`AskUserQuestion` 是**同步阻塞工具**——调用它后宿主会渲染选择器、等用户选项确定后**作为 tool result 返回给你**（你看到的不是 user prompt，是 tool output）。**你在同一 turn 内继续处理**，无需 end turn。

selector 模板末尾的"调用工具后立即 end turn 等待用户选择"是**历史措辞**，准确语义是：

1. 调 `AskUserQuestion` → 工具阻塞等用户选项
2. 拿到 user 选项（tool result）→ **同一 turn 内**按 selector 模板的「**用户选定后流程**」段继续推进（每个 selector 模板末尾都有这段；2026-05-22 起统一加上）
...
`spec_session.py phase-transition` 切换 spec 的**内部 phase**（intake→requirements→design→tasks→implementation→acceptance→iteration），spec 仍然在 `mode=active`，session 仍然持锁，hook 继续每轮注入提醒。**只有 `/specode:end`** 才让 session `mode=ended`、释锁、停 hook。

**严禁**说 "Spec 流程完成！现在退出 spec 模式，开始编码实现" / "spec 已完成，开始 coding" 这类话 —— `tasks-execution` selector 选了 "用 task-swarm" 或 "顺序执行" 后是 **phase-transition → implementation**，**仍在 spec 模式**；implementation phase 期间 hook 继续每轮注入「📝 文档优先」「🔄 代码-文档同步」「🪧 状态行 footer」「⛔ 仍处于 spec 模式」四条提醒，主代理改代码前后必须按 §Code-Doc Sync Reminders 同步 `tasks.md` / `implementation-log.md` / `design.md`。

「spec 流程完成 / 退出 spec 模式」这种判断**只有**满足以下两条之一时才成立：
- 用户主动输入 `/specode:end`
- `acceptance-gate` selector 选「验收通过，进入 iteration」**且** `iteration-scope` 全不选 / ESC（视为本轮 iteration 取消，留 acceptance phase 等用户决定）—— 即使这种情况也只是「无活跃 phase 推进」，**spec 仍 active 直到 /specode:end**
...
7. **`requirements.md` / `bugfix.md` / `design.md` / `tasks.md` 4 份核心文档由主代理直接生成**——按 §Spec 文档生成 规则：Read `${CLAUDE_PLUGIN_ROOT}/assets/templates/<phase>.md` 模板作骨架 + 按 `source_text`（用户原始需求）填空 + Write 到 `<spec-dir>/<phase>.md`。0.10.11 起 `spec-writer` subagent 已删除（subagent 拿不到主代理的 SKILL 上下文 + 用户原始需求 + 流程状态，hallucinate 通用模板内容；主代理直接写质量更高、上下文连续）。`implementation-log.md` 同样由主代理追加。
