...
文件优先的规范驱动工作流。`requirements.md` / `bugfix.md` / `design.md` / `implementation-log.md` 是事实源；代码改动总是滞后于文档落地。所有 hook 都是**提示式注入**，永远不阻断；hook 注入失败或缺失时，本 SKILL.md 的硬约束仍然完整有效。
...
3 份核心 spec 文档（`requirements.md` / `bugfix.md` / `design.md`）由**主代理本身**生成，**不** fork subagent（M4 起 specode 不再产 `tasks.md`）。统一规则：
...
- **`## ` 二级标题逐字保留**：不允许改名、合并、拆分、调整顺序、新增不在模板里的章节。`### ` 三级标题里以 `需求 N` 这种为模板的"动态前缀"——可按需重复（一份需求文档可以有 `### 需求 1` / `### 需求 2` / …），但前缀本身（"需求 "）不可改。
...
**后置兜底**：每次 phase-transition、acceptance 都会跑 `spec_lint.py rule_template_structure`，对 3 份核心文档做章节集合比对——缺 mandatory / 多 unknown 章节都会报 `[WARN][tmpl]`，hook 把 WARNING 注入下一轮，主代理需要立即修。详细规则与原因见 `references/templates.md` §模板章节铁律。
...
实际反例：选定 `doc-confirm-design` 后切到 delegated phase，hook 下一轮会注入 `delegation` selector 模板（3 个固定选项：委托 task-swarm 执行（多 agent 并发）/ specode 自执行（顺序，单 agent）/ 暂停或调整 design）；如果同一 turn 内主代理主动呈现，**必须查 `_selectors.py` SELECTOR_PROMPTS['delegation']** 拿到 3 个固定选项原文，不要 invent 成 2 选项简化版。
...
`spec_session.py phase-transition` 切换 spec 的**内部 phase**（intake→requirements→design→delegated→acceptance→iteration），spec 仍然在 `mode=active`，session 仍然持锁，hook 继续每轮注入「📝 文档优先」「🔄 代码-文档同步」「🪧 状态行 footer」「⛔ 仍处于 spec 模式」四条提醒。**只有 `/specode:end`** 才让 session `mode=ended`、释锁、停 hook。

**严禁**说 "Spec 流程完成！现在退出 spec 模式，开始编码实现" / "spec 已完成" —— `delegation` 选 "委托 task-swarm 执行" / "specode 自执行" 后是 `phase-transition → delegated`，**仍在 spec 模式**，委托 / 自执行的代码改动前后都必须按 §Code-Doc Sync Reminders 同步 `implementation-log.md` / `design.md`；委托态 resolve done 后回 `phase-transition → acceptance`，全程不退出 spec 模式。
...
| `delegation` | A | design 完成、进入 delegated 后（委托 / 自执行 / 调整 design） | 执行方式 |
...
| `design.md` | 架构 / 接口 / 数据模型决策（delegated 委托时即任务拆分的事实源） |
| `implementation-log.md` | 委托 / 自执行期间记录设计偏离 / 关键决策（可选；≥30 字） |

> M4 起 specode 不再产 `tasks.md`——委托模式下任务拆分进 task-swarm 的 `pipeline.yml`（由主代理读 `design.md` 生成）；自执行降级直接照 `design.md` 实现。
...
## Delegated / 委托执行（delegated 阶段）

M4 起 `tasks` + `implementation` 两个旧 phase 合并为单一 `delegated` 阶段：**specode 不再单独产 `tasks.md`**——任务拆分归 task-swarm 的 planner（委托时由主代理读 `design.md` 生成 `pipeline.yml`）。design 确认后 `phase-transition --from design --to delegated`，hook 下一轮注入 `delegation` 选择器（3 条出路）。

### 委托 task-swarm 执行（多 agent 并发）

task-swarm 是**独立 plugin**，specode **零 import** 它、也不知道它的安装路径——一切调用走 task-swarm 自带的 `/task-swarm` 命令 / 其 `run.sh`（其 `$CLAUDE_PLUGIN_ROOT` 自解析）。委托是 C 模型：主代理在**当前会话**连续驱动整条编排，不 end turn 等用户逐步：

1. **Read task-swarm SKILL.md**（拿到它的 plan→fork→advance→writeback→resolve 状态机协议）。
2. **读本 spec `design.md`** → 生成 `<spec-dir>/pipeline.yml`（任务组 / `@writes` 文件 / `needs` 拓扑）。
3. **呈现 yml 摘要给用户过目**（任务组数 / 同文件冲突 / 拓扑），用户确认后再 init。
4. `task_swarm.py init --pipeline <yml> --workdir <项目根> --spec-id <slug> --spec-dir <spec-dir> --session <id>`（经 task-swarm 的 run.sh 包装；`--spec-id` / `--spec-dir` 让 task-swarm 回溯到本 spec）。
5. 拿到 `run_id` → 调 `spec_session.py set-delegated-run --run-id <rid> --session <id>`（在 sessions 文件写 `delegated_run_id` 标记，委托态 footer / Stop 会提醒最终回 acceptance）。
6. **按 task-swarm SKILL 驱动** plan → fork → advance → writeback → resolve 直到 resolve `done`。
7. resolve done 后：`spec_session.py set-delegated-run --clear --session <id>` + `phase-transition --from delegated --to acceptance`。

**未装 task-swarm**（`init` 调用失败 / `/task-swarm` 不可用）→ 告知用户安装，或当场改走下面的 specode 自执行降级。

### specode 自执行降级（顺序，单 agent）

不依赖 task-swarm（未安装、或小改动不值得起 swarm）。主代理按 `design.md` **直接 Edit 代码 + 自验**（**无 `tasks.md`**，不起 subagent）；完成后 `phase-transition --from delegated --to acceptance`。

### 零硬依赖声明

specode 对 task-swarm 是**纯运行期、可选**耦合：不 import 其任何模块、不假设其安装路径，只在用户选「委托」时调它的 `/task-swarm` 命令（命令自带 plugin root）。task-swarm 缺席不影响 specode 任何流程——直接降级自执行即可。

无论委托还是自执行，代码改动前后都必须按 §Code-Doc Sync Reminders 同步 `implementation-log.md` / `design.md`，**仍在 spec 模式**直到 `/specode:end`。
...
7. **`requirements.md` / `bugfix.md` / `design.md` 3 份核心文档由主代理直接生成**——Read `${CLAUDE_PLUGIN_ROOT}/assets/templates/<phase>.md` 骨架 + 按 `source_text` 填空 + Write 到 `<spec-dir>/<phase>.md`。`implementation-log.md` 同样由主代理追加。M4 起 specode 不再产 `tasks.md`（委托模式归 task-swarm planner，自执行降级照 `design.md` 实现）。详细规则与 spec-writer subagent 已删除的原因见 §Spec 文档生成。
