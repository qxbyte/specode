...
| `delegated` | 委托 task-swarm 或 specode 自执行（M4 起 `tasks` + `implementation` 合并为此单一阶段，不再产 tasks.md） | design 确认后 |
| `acceptance` | 代码完成，跑 UAT | delegated 阶段实现完成 |
...
| Phase | 一句话 | 允许操作 |
|---|---|---|
| `delegated` | 委托 task-swarm 或 specode 自执行写代码兑现 design | 编辑代码、追加 implementation-log（M4 起无 tasks.md） |
...
iteration ← 默认停留状态
 ...
 │ → 走 doc-confirm-requirements → design
 │ → delegated（委托 task-swarm 或 specode 自执行）→ acceptance
 │ → acceptance-gate 通过 → 自动回到 iteration（round +1）
 │
 ├─ 用户："改一下 acceptance 里某条规则"
 │ → 直接编辑 design.md 「测试策略」/「正确性属性 → 验证：需求 x.y」对应行
 │ →（不需走完整子循环 —— 视为微调）
 │
 ├─ 用户："只重跑测试"
 │ → 不改文档，跑 design.md 测试策略 / task-swarm report 中的复现命令
...
- 可重走 requirements → design → delegated 子循环。
...
| `design.md` | 原节内**可修改**；必须在 `## 变更历史` 节追加 `### 迭代 N` 子节（无此节则创建），说明本轮架构 / 接口 / 数据模型变更。原节内修改的地方留 `<!-- [迭代 N] -->` 注释标记。任务拆分与测试要点也收敛到 design.md（M4 起无 tasks.md）。 |
...
1. 改 requirements
2. 改 design
3. 改 tasks（重新委托 task-swarm 或 specode 自执行；M4 起无 tasks.md，任务拆分归 design.md / pipeline.yml）
4. 重跑测试
...
- 核心文档的章节模板 → `references/templates.md`。
