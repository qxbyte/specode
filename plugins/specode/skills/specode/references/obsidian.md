...
多 vault 时**调用 `AskUserQuestion` 工具**呈现选择器（类型 A 单列单选；详见 `references/prompts.md`）。例（动态构造，path 来自 `spec_vault.py detect` 输出）：
...
AskUserQuestion(questions=[{
    ...
    "multiSelect": False,
    "options": [
        # 最多 4 项；超过 4 个 vault 时取 ts 最新的 4 个，其余引导用户用 /specode:spec --set-vault <path>
        {"label": "/Users/alice/Documents/main-vault",
         "description": "open=true，最近活动 2026-05-19 09:55"},
        {"label": "/Users/alice/Documents/research-vault",
         "description": "open=true，最近活动 2026-05-18 22:10"},
        {"label": "/Users/alice/Documents/archive-vault",
         "description": "open=false，最近活动 2026-04-01 15:00"},
    ],
}])
```
...
- **必须调用 `AskUserQuestion` 工具**，禁止在 chat 输出 markdown 列表让用户回复编号。
- `multiSelect=false`。
- 选项数 ≤ 4（工具上限）；超过 4 个 vault 时取最近 `ts` 的 4 个 + 在 chat 说明剩余可通过 `/specode:spec --set-vault <path>` 设置。
- **不**给"（推荐）"——用户必须显式选。
- "Other" 由工具自动提供，用户可在 Other 中输入自定义路径；ESC 取消。
- 不要列出 `spec_vault.py detect` 之外的路径。
- 工具返回后调 `spec_vault.py set --vault <chosen-path>` 持久化。
...
# → {"root": "...", "source": "...", "specs": [
# {"slug": "...", "phase": "...", "lock_state": "held|free|stale", "holder": "abc12345",
# "displayName": "...", "iterationRound": N, "mtimes": {...}},
# ...
# ], "ok": true}
```

`list-specs` 的输出已经聚合了 spec 元数据 + 锁状态 + 文档 mtime——不需要再去读各 spec 的 `.config.json`。把这份 JSON 转成 `AskUserQuestion` 工具调用呈现（类型 A 单列单选）：

```python
# 先在 chat 写 1-2 行上下文："找到 N 个可继续 spec，当前 root：<root>（source=<source>）"，
# 然后调工具：
...
AskUserQuestion(questions=[{
    ...
    "multiSelect": False,
    "options": [
        # 最多 4 项；超过 4 个时取按 last_heartbeat_at 最近的 4 个，其余在 chat 引导用户手动 /specode:continue <slug>
        {"label": "<slug-1>",
...
约束：
- `multiSelect=false`；选项数 ≤ 4。
- **不**给"（推荐）"——用户必须显式选。
- "Other" 由工具自动提供，允许用户输入 spec slug 或路径作为自定义答案。
- 工具返回后下一轮进入 `/specode:continue <slug>` 流程（详见 `references/workflow.md` §9.2）。
- list-specs 返回 `specs: []` → **不**调工具，直接在 chat 提示"该 root 下没有 spec，运行 `/specode:spec <需求>` 创建"。
