...
3. **若有多个 `open=true` 的 vault**：让用户在 chat 里**回复编号**选择（详见 §3）。
...
多 vault 时按 §3.7.1 类型 A 单列单选骨架（详见 `references/prompts.md`）输出。例：

```text
=== 选择 Obsidian vault ===
当前阶段：vault-binding
检测到多个 vault；请挑选 specode 使用的目录。

1. /Users/alice/Documents/main-vault
 open=true，最近活动 2026-05-19 09:55
2. /Users/alice/Documents/research-vault
 open=true，最近活动 2026-05-18 22:10
3. /Users/alice/Documents/archive-vault
 open=false，最近活动 2026-04-01 15:00
4. Type something
 输入自定义 vault 绝对路径
5. Chat about this
 就这个选择讨论，不下决定
...
硬约束（与 SKILL.md §Selectors 一致）：

- 必须以 `AWAITING_USER_CHOICE` 单独一行结尾，输出后 end turn。
- 保留 `Type something` + `Chat about this` 两个保留位。
- **不**给"（推荐）"默认——用户必须显式选；即使第一个看起来最近。
- 选项数量动态（vault 数 + 2 个保留位）；不要列出 `spec_vault.py detect` 之外的路径。
...
`list-specs` 的输出已经聚合了 spec 元数据 + 锁状态 + 文档 mtime——不需要再去读各 spec 的 `.config.json`。把这份 JSON 转成"列表 + 用户回复编号"形式呈现（参考 §3 的骨架，但**不必加 `AWAITING_USER_CHOICE` sentinel**——这里只是 informational 列表，模型 end turn 等用户回复编号即可）：

```text
=== 可继续的 spec ===
当前 root：<root>（source=<source>）

 1. <slug> ← <displayName>
 phase=<phase> 迭代=N lock=<held|free|stale>（持有者: <holder 前 8 位>）
 最近修改：requirements <ts>, tasks <ts>
 2. ...

请回复编号或 slug，或运行 `/specode:end` 退出。
```

用户回复编号后，下一轮进入 `/specode:continue <slug>` 流程（详见 `references/workflow.md` §9.2）。
