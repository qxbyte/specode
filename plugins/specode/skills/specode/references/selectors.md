...
本文件给出全部 8 个固定场景的 (类型 → AskUserQuestion 参数)；spec_session/_selectors.py 的 `SELECTOR_PROMPTS` 字典是这些模板的运行时常量库。`doc-confirm-tasks` 已合并进 `tasks-execution`（详见 §A4）。
...
3 个变体骨架一致，只 `<filename>` / question / header / 简报要点重心 / 文档路径不同；运行时按 `pending_selector` 命中对应 key 选模板。`doc-confirm-tasks` 已合并进 `tasks-execution`（§A4）。
